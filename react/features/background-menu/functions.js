 ...
 */
export function getBlurEffect(name: String) {
    const ns = getJitsiMeetGlobalNS();

    if (ns.effects && ns.effects.createBlurEffect) {
        return ns.effects.createBlurEffect(name);
    }

    return loadScript('libs/video-blur-effect.min.js').then(() => ns.effects.createBlurEffect(name));
}
